...
namespace HelmetDetection {
  ...
  gint helmet_count = 0;
  gint no_helmet_count = 0;

  void
  Helmet::update_fps(gint id) {
  ...
  int
  Helmet::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  Helmet::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Helmet::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
    ...
    offset = snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "Source: %d | FPS: %d | Helmets: %d | No Helmets: %d ",
                      frame_meta->source_id, fps[frame_meta->source_id].display_fps, helmet_count, no_helmet_count);
  ...
  GstPadProbeReturn
  Helmet::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      ...
      for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
        ...
        if(class_index == BikeHelmet) {
          changeBBoxColor(obj_meta, 1, 0.0, 1.0, 0.0, 0.25);
        }

        if(class_index == NoHelmet || class_index == Hair) {
  ...
  gboolean
  Helmet::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  Helmet::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  Helmet::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  Helmet::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  Helmet::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  Helmet::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  Helmet::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        HelmetDetection::Helmet::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", HelmetDetection::Helmet::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!HelmetDetection::Helmet::set_tracker_properties(nvtracker)) {
  ...
  void Helmet::setPaths(guint num_sources) {
...
int main(int argc, char *argv[]) {
  ...
  ctx = g_option_context_new("Helmet DeepStream App");
  group = g_option_group_new("Helmet", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, helmet.entries);
  ...
  gint sources = helmet.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  int fail_safe = helmet.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, helmet.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      helmet.tiler_src_pad_buffer_probe, NULL, NULL);
  }
